...
function draw() {
    ...
    let gravity2 = createVector(0, 0.1 * ball2.mass)
    let wind = createVector(0.1, 0);

    ball2.show();
    ball.show();
    ball.applyForce(gravity);
    ball2.applyForce(gravity2);
    if(mouseIsPressed) {
...
class Mover {
    ...
    checkEdges() {
        ...
        if(this.position.x >= (width * 3) / 4) {
            let scale = map(width - this.position.x, 1/4 * width, 0, 0, 1);
            let invisForce = createVector(-scale, 0);
            this.applyForce(invisForce);
        } 
        if(this.position.x <= width / 4) {
            let scale = map(this.position.x, 1/4 * width, 0, 0, 1);
            let invisForce = createVector(scale, 0);
            this.applyForce(invisForce);
        }
    }
